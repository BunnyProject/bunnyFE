...
function HomeScreen({navigation}: HomeScreenProps) {
  ...
  const loadMonthlyTarget = useCallback(async () => {
    try {
      ...
      setMonthlyGoal(savedTarget ? Number(savedTarget) : 0);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    intervalRef.current = setInterval(() => {
      ...
      if (now > end) {
        ...
        leftTime = (end.getTime() - start.getTime()) / 1000;
      } else {
        elapsedSeconds = Math.floor((now.getTime() - start.getTime()) / 1000);
        earning = Math.floor((elapsedSeconds / 60) * ratePerMinute);
        leftTime = Math.floor((end.getTime() - now.getTime()) / 1000);
      }
  ...
  const progress =
    start && end
      ? Math.max(0, Math.min(elapsedTime / ((end.getTime() - start.getTime()) / 1000), 1))
      : 0;

  return (
    <View style={styles.container}>
      <Header month={month} currentDayOfMonth={currentDayOfMonth} dayOfWeekStr={dayOfWeekStr} />
      <CircleProgress
        progress={progress}
        formattedElapsedTime={formatTime(elapsedTime)}
        formattedTimeLeft={timeLeft <= 0 ? formatTime(0) : formatTime(timeLeft)}
        earnings={earnings}
      />
      ...
      />
      <SavingsModal isVisible={isModalVisible} onClose={() => setIsModalVisible(false)} />
    </View>
  );
}

const Header = memo(({month, currentDayOfMonth, dayOfWeekStr}: any) => (
...
const CircleProgress = memo(({progress, formattedElapsedTime, formattedTimeLeft, earnings}: any) => (
  <View style={styles.circleContainer}>
    <Svg width={200} height={200} viewBox="0 0 200 200" style={{transform: [{rotate: '-90deg'}]}}>
      ...
      <Circle cx="100" cy="100" r="90" stroke="#f4f4f4" strokeWidth="15" fill="none" />
      <Circle
        cx="100"
        cy="100"
        r="90"
    ...
    <View style={styles.progressTextContainer}>
      ...
      <Text style={styles.progressSubText}>퇴근까지 {'\n'} {formattedTimeLeft}</Text>
    </View>
...
const GoalProgress = memo(({monthlyGoal, todayEarnings, onEditTarget}: any) => (
  <View style={styles.monthlyGoalContainer}>
    <View style={styles.goalTextContainer}>
      ...
      <View style={styles.goalTextWrapper}>
        ...
        <TouchableOpacity style={styles.iconButton} onPress={onEditTarget}>
    ...
    <View style={styles.progressBarContainer}>
      <Svg height="20" width="100%">
        ...
        <Rect x="0" y="0" width="100%" height="20" fill="#f4f4f4" />
        <Rect
          x="0"
          y="0"
